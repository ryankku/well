...
const checkoutStatusHtml = fs.readFileSync(path.join(rootDir, 'checkout-status.html'), 'utf-8');
const productHtml = fs.readFileSync(path.join(rootDir, 'product.html'), 'utf-8');

const apiTemplate = `const express = require('express');
...
// HTML PAGES EMBEDDED
const STOREFRONT_HTML = \`__INDEX_HTML__\`;
const ADMIN_HTML = \`__ADMIN_HTML__\`;
const CHECKOUT_HTML = \`__CHECKOUT_HTML__\`;
const CHECKOUT_STATUS_HTML = \`__CHECKOUT_STATUS_HTML__\`;
const PRODUCT_HTML = \`__PRODUCT_HTML__\`;
...
const finalApiCode = apiTemplate
  .replace('__INDEX_HTML__', indexHtml.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\${/g, '\\${'))
  .replace('__ADMIN_HTML__', adminHtml.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\${/g, '\\${'))
  .replace('__CHECKOUT_HTML__', checkoutHtml.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\${/g, '\\${'))
  .replace('__CHECKOUT_STATUS_HTML__', checkoutStatusHtml.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\${/g, '\\${'))
  .replace('__PRODUCT_HTML__', productHtml.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\${/g, '\\${'));

fs.writeFileSync(path.join(rootDir, 'api', 'index.js'), finalApiCode, 'utf-8');
console.log('Created standalone api/index.js with embedded HTML pages!');
